...
const BookTable = ({listbook,loading}) => {
            ...
            renderCell: (params) => {
                return(
                    <Tooltip placement="bottom" title={edit_book_tooltip}>
                        <Link to={`/admin/books/edit/${params.row.id}`}>
            ...
            renderCell: (params) => {
                return(
                    <Tooltip placement="bottom" title={status_book_tooltip((params.row.Tinh_Trang === 1)?"Đang hiện":"Đang ẩn")}>
                        <div className="icon-hidden-book" onClick={()=>handleSetStatus(params.row.id, params.row.Tinh_Trang)}>
